...
public class RouteActivity extends Activity implements OnMarkerClickListener,
	...
	private int routeType = 1;// 1代表公交模式�?代表驾车模式�?代表步行模式
	...
	public void startSearchResult() {
		...
		if (startPoint != null && strStart.equals("地图上的起点")) {
			...
			showProgressDialog();
			startSearchQuery = new PoiSearch.Query(strStart, "", "010"); // 第一个参数表示查询关键字，第二参数表示poi搜索类型，第三个参数表示城市区号或�?城市�?
			startSearchQuery.setPageNum(0);// 设置查询第几页，第一页从0�?��
			startSearchQuery.setPageSize(20);// 设置每页返回多少条数�?
	...
	public void endSearchResult() {
		...
		if (endPoint != null && strEnd.equals("地图上的终点")) {
			...
			showProgressDialog();
			endSearchQuery = new PoiSearch.Query(strEnd, "", "010"); // 第一个参数表示查询关键字，第二参数表示poi搜索类型，第三个参数表示城市区号或�?城市�?
			endSearchQuery.setPageNum(0);// 设置查询第几页，第一页从0�?��
			endSearchQuery.setPageSize(20);// 设置每页返回多少条数�?
	...
	public void searchRouteResult(LatLonPoint startPoint, LatLonPoint endPoint) {
		...
		if (routeType == 1) {// 公交路径规划
			BusRouteQuery query = new BusRouteQuery(fromAndTo, busMode, "北京", 0);// 第一个参数表示路径规划的起点和终点，第二个参数表示公交查询模式，第三个参数表示公交查询城市区号，第四个参数表示是否计算夜班车�?表示不计�?
			routeSearch.calculateBusRouteAsyn(query);// 异步路径规划公交模式查询
		} else if (routeType == 2) {// 驾车路径规划
			DriveRouteQuery query = new DriveRouteQuery(fromAndTo, drivingMode,
					null, null, "");// 第一个参数表示路径规划的起点和终点，第二个参数表示驾车模式，第三个参数表示�?经点，第四个参数表示避让区域，第五个参数表示避让道路
	...
	public void onPoiSearched(PoiResult result, int rCode) {
		...
		if (rCode == 0) {// 返回成功
			if (result != null && result.getQuery() != null
					&& result.getPois() != null && result.getPois().size() > 0) {// 搜索poi的结�?
